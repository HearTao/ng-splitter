...
export function analyzeServices(
  ...
  Array.from(analyzedModules.ngModules.values())
    ...
    .forEach(x => {
      x.providers
        .filter(provider =>
          referenceIsValidFile(tsProgram, provider.token.identifier!.reference)
        )
        .forEach(provider => {
          appendToSetMap(
            declarationMap,
            provider.useClass!.reference,
            x.type.reference
          )
        })
    })
...
export function analyzeServicesUsage(
  result: PerformCompilationResult
): ServicesUsageInfo {
  const analyzedModules = result.program!._analyzedModules
    ...
    .forEach(x => {
      ...
      x.providers.forEach(provider => {
        appendToSetMap(
          ...
          provider.token.identifier!.reference,
          x.type.reference
        )

        const parameters: StaticSymbol[][] = reflector.parameters(
          provider.token.identifier!.reference
        )
        parameters
          .flat()
...
export function analyzeComponentUsage(
  ...
  Array.from(templateAstCache.entries())
    .map(([key, value]) => value.template.map(temp => [key, temp] as const))
    .flat()
    .filter(
      ([key, value]) =>
        value &&
        !key.name.endsWith('_Host') &&
        referenceIsValidFile(tsProgram, key)
    )
    .map(([key, value]) => {
      ...
      context.elements.forEach(x => {
        x.providers
          .filter(
            x =>
              x.providerType === ProviderAstType.Component ||
              x.providerType === ProviderAstType.Directive
          )
          .filter(x =>
            referenceIsValidFile(tsProgram, x.token.identifier!.reference)
          )
          .forEach(componentOrDirective => {
            appendToSetMap(
              componentUsageMap,
              componentOrDirective.token.identifier!.reference,
              key
            )
          })
      })
      ...
      context.directives
        .filter(x =>
          referenceIsValidFile(tsProgram, x.directive.type.reference)
        )
        .forEach(x => {
  ...
  Array.from(templateAstCache.entries())
    .map(([key, value]) => value.pipes.map(pipe => [key, pipe] as const))
    .flat()
    .filter(([key]) => referenceIsValidFile(tsProgram, key))
    .map(([key, value]) => {
